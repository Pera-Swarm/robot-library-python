...
class Motion:
    ...
    def move_distance(self, speed, distance):
        ...
        self.client.publish(
            f"robots/{self.robot_id}/motion", f"MOVE {speed} {distance}"
        )
    ...
    def move(self, left_speed, right_speed, duration):
        ...
        self.client.publish(
            f"robots/{self.robot_id}/motion",
            f"DRIVE {left_speed} {right_speed} {duration}",
        )
    ...
    def __init__(self, robot_id, x=0, y=0, heading=0, mqtt_config=None):
        ...
        if mqtt_config:
            ...
            def loop(self):
                ...
                print(
                    f"Robot {self.robot_id} running at position ({self.x}, {self.y}) heading {self.heading}"
                )
                pass
